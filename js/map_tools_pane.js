...

{
  ...
  const GridPane = class {
    constructor (mapPane) {
      ...
      displayGridCheckbox.addEventListener('change', function () {
      ...
      horizontalSpacing.addEventListener('change', function () {
      ...
      verticalSpacing.addEventListener('change', function () {
        var value = this.value | 0
        if (value < 1) {
          this.value = value = 1
        }
  ...
  const ScalingPane = class {
    constructor (mapPane, toolsPane) {
      ...
      addButton('map_scale_positions_btn', function () {
        mapPane.map.scalePositions(scalingFactor.value | 0)
        emitEvent()
      })
      addButton('map_scale_all_btn', function () {
        mapPane.map.scaleAll(scalingFactor.value | 0)
        emitEvent()
      })
      addButton('map_scale_selected_object_btn', function () {
  ...
  const FilteringPane = class {
    constructor (mapPane, objectListBox) {
      ...
      filterObjectsCheckbox.addEventListener('change', function () {
  ...
  const clazz = class {
    constructor (mapPane, objectListBox) {
      ...
      this.gridBtn = addButton('map_grid_tool_btn', () => {
        this.togglePane(this.mapGridToolPane)
      })

      this.scalingBtn = addButton('map_scaling_tool_btn', () => {
        this.togglePane(this.mapScalingToolPane)
      })

      this.filteringBtn = addButton('map_filtering_tool_btn', () => {
